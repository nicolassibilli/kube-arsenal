...
# =============================================================================
# Purpose:
#   Install the Kubernetes / DevOps / policy / security tools that were present
#   in the validated workstation check.
#
# Target:
...
#
# Modes:
#   -i, --install  Install and configure the workstation tools.
...
show_help() {
  cat <<'EOF'
SYNOPSIS
  install-k8s-admin-workstation.sh [OPTIONS]

DESCRIPTION
  Install, configure, or check a Kubernetes administration workstation.

OPTIONS
  -i, --install
      Install and configure all validated Kubernetes workstation tools.
...
EXAMPLES
  ...
  INSTALL_DOCKER=true ./install-k8s-admin-workstation.sh -i
EOF
}
...
write_kube_shell_config() {
  local rc_file="$1"
  local start_marker="# >>> k8s-admin-workstation managed block >>>"
  local end_marker="# <<< k8s-admin-workstation managed block <<<"
  local tmp_file
...
echo
echo "Then run the integrated check mode:"
echo "  $0 --check"
